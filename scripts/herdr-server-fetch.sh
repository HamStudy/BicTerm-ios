...
#
# Downloads the herdr v0.9.0 macOS aarch64 release asset into
# Fixtures/run/herdr/herdr (gitignored) and verifies it against the committed
# sha256 lockfile Fixtures/herdr/server-0.9.0.sha256. Idempotent: when the
# on-disk binary already matches the lockfile, nothing is downloaded.
#
# Asset: https://github.com/herdrdev/herdr/releases/download/v0.9.0/herdr-macos-aarch64
# (asset name verified against the v0.9.0 release page with curl)
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
RUN="$ROOT/Fixtures/run/herdr"
BIN="$RUN/herdr"
LOCK="$ROOT/Fixtures/herdr/server-0.9.0.sha256"
...
if [ -f "$BIN" ]; then
  ...
  if [ "$actual" = "$expected" ]; then
    echo "herdr-server-fetch: $BIN already matches server-0.9.0.sha256 (skip)"
    exit 0
  fi
...
mv "$partial" "$BIN"
echo "herdr-server-fetch: fetched herdr 0.9.0 -> $BIN (sha ok)"

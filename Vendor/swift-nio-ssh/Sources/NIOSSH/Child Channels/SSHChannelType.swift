...
public enum SSHChannelType: Equatable, Sendable {
    ...
    // BICTERM-PATCH hunk 7: "auth-agent@openssh.com" — a server-initiated
    // channel carrying the OpenSSH agent protocol (PROTOCOL.agent) back to a
    // client that requested agent forwarding. Client-side only; carries no
...
extension SSHChannelType {
    internal init(_ message: SSHMessage.ChannelOpenMessage) {
        ...
        // BICTERM-PATCH hunk 8: map parsed auth-agent opens to the public type.
        case .authAgent:
            self = .authAgent
        }
    }
...
extension SSHMessage.ChannelOpenMessage.ChannelType {
    internal init(_ type: SSHChannelType) {
        ...
        // BICTERM-PATCH hunk 9: reverse mapping (exhaustiveness; clients
        // never originate auth-agent opens).
        case .authAgent:
            self = .authAgent
        }
    }

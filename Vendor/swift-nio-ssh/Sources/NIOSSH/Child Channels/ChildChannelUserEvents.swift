...
public enum SSHChannelRequestEvent: Sendable {
    ...
    // BICTERM-PATCH hunk 10a: "auth-agent-req@openssh.com" — asks the server
    // to enable agent forwarding for this session (PROTOCOL.agent). OpenSSH's
    // own client sends it with wantReply = false and treats denial as
    // non-fatal. Carries no payload.
...
extension SSHChannelRequestEvent {
    ...
    internal static func fromMessage(_ message: SSHMessage.ChannelRequestMessage) -> (any Sendable)? {
        ...
        // BICTERM-PATCH hunk 11: clients never legitimately receive
        // auth-agent-req; returning nil makes a wantReply copy fail with
        // SSH_MSG_CHANNEL_FAILURE (SSHChildChannel.handleInboundChannelRequest).
        case .authAgentReq:
            return nil
        case .unknown:
            return nil
...
extension SSHMessage {
    ...
    // BICTERM-PATCH hunk 10b: event → wire message for auth-agent-req.

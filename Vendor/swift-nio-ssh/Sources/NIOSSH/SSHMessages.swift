...
extension SSHMessage {
    ...
    struct ChannelOpenMessage: Equatable {
        ...
        enum ChannelType: Equatable {
            ...
            // BICTERM-PATCH hunk 1: OpenSSH agent forwarding channel
            // ("auth-agent@openssh.com", PROTOCOL.agent). Carries no
            // type-specific payload in the channel open.
            case authAgent
        }
    ...
    struct ChannelRequestMessage: Equatable {
        ...
        enum RequestType: Equatable {
            ...
            // BICTERM-PATCH hunk 5: OpenSSH agent forwarding channel request
            // ("auth-agent-req@openssh.com"). Carries no payload.
            case authAgentReq
            case unknown
        }
...
extension ByteBuffer {
    ...
    mutating func readChannelOpenMessage() throws -> SSHMessage.ChannelOpenMessage? {
        try self.rewindOnNilOrError { `self` in
            ...
            // BICTERM-PATCH hunk 2: parse "auth-agent@openssh.com" channel
            // opens instead of throwing unknownPacketType (which killed the
            // whole connection). No type-specific payload per PROTOCOL.agent.
    ...
    mutating func readChannelRequestMessage() throws -> SSHMessage.ChannelRequestMessage? {
        try self.rewindOnNilOrError { `self` in
            ...
            // BICTERM-PATCH hunk 6a: parse "auth-agent-req@openssh.com"
            // channel requests (no payload).
...
extension ByteBuffer {
    ...
    mutating func writeChannelOpenMessage(_ message: SSHMessage.ChannelOpenMessage) -> Int {
        ...
        // BICTERM-PATCH hunk 3: serialize "auth-agent@openssh.com" opens.
        case .authAgent:
        ...
        // BICTERM-PATCH hunk 4: "auth-agent@openssh.com" opens have no
    ...
    mutating func writeChannelRequestMessage(_ message: SSHMessage.ChannelRequestMessage) -> Int {
        ...
        // BICTERM-PATCH hunk 6b: serialize "auth-agent-req@openssh.com".
        case .authAgentReq:
        ...
        case .signal(let name):
            writtenBytes += self.writeSSHString(name.utf8)
        // BICTERM-PATCH hunk 6c: "auth-agent-req@openssh.com" has no payload.
        case .authAgentReq:
            break
        case .unknown:
            preconditionFailure()

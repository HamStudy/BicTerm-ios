...
| `0008-embed-redial-attention-machines-on-catalog-reload.patch` | `src/client/catalog_reload.rs` (+145), `src/client/endpoint/supervisor.rs` (+108), `src/client/shell/endpoints.rs` (+12) | Extends `bicterm-embed`: every machine-catalog reload — including a rewrite of `endpoints.json` with identical content, which the embedding host uses as a "retry now" signal — re-arms the machines the shell reports as needing attention (`EndpointSupervisors::rearm_attention` sets `attempts = 0`, `next_attempt = now`, so the next `spawn_due` tick redials them with a fresh backoff budget). Upstream `reconcile_profiles` keeps an unchanged profile's `ReconnectState`, so an Attention machine otherwise sits at `next_attempt = None` forever; `record_status` itself never re-arms, so the pinned no-retry-storm test (`ssh_recovery_rejects_stale_generations_and_stops_retries_for_attention`) stays green and Attention still stops retries until the next reload. Attention is read from the shell's endpoint status (which mirrors the supervisor's `record_status` outcome); Online/Disabled/Reconnecting/Connecting endpoints and in-flight attempts are untouched, and generation fencing is unchanged. `watch_profiles` treats a catalog file rewrite (mtime change) as a reload event so the identical-content re-seed reaches `apply_profiles`; stock builds keep upstream's pure content-diff detection (the stock `catalog_stamp_changed` is a constant `false` and never touches the filesystem) | no (BicTerm embed; the reload-as-retry contract is the host's) |
...
stock builds. Same shape for `bicterm-embed` (patches 0005, 0006, 0007, and 0008):
without it `run_client` keeps the upstream `std::process::exit(1)` contract,
every terminal-setup/geometry path is byte-for-byte upstream, and the Local
endpoint is always present (`HERDR_EMBED_HIDE_LOCAL` is never read); only
the `herdr-ios-embed` staticlib selects the error-return, socketpair-stdio,
and hidden-Local seams; catalog reloads never re-arm Attention machines
(`catalog_stamp_changed` is a constant `false`, so identical-content
rewrites fire no event and no supervisor re-arm exists).

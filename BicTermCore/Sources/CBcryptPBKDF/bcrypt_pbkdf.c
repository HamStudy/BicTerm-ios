...
static void
secure_bzero(void *buffer, size_t length)
{
    ...
    uint8_t *key, size_t keylen, unsigned int rounds)
{
	...
	if (pass == NULL || salt == NULL || key == NULL ||
	    passlen == 0 || saltlen == 0 || keylen == 0 ||
	    keylen > sizeof(out) * sizeof(out) || saltlen > 1<<20)
		goto bad;
	if (passlen > UINT_MAX || saltlen + 4 > UINT_MAX)
		goto bad;
	...
	/* collapse password */
	CC_SHA512(pass, (CC_LONG)passlen, sha2pass);
	...
	for (count = 1; keylen > 0; count++) {
		...
		CC_SHA512(countsalt, (CC_LONG)(saltlen + 4), sha2salt);
	...
	/* zap */
	secure_bzero(countsalt, saltlen + 4);
	free(countsalt);
	secure_bzero(out, sizeof(out));
	secure_bzero(tmpout, sizeof(tmpout));

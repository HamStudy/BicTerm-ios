...
- README dependency inventory (docs task): the real toolchain surface is Xcode 26 + iOS 26.3 runtime, xcodegen, Go (go.mod pins 1.26.5, auto-downloaded), python3 (stdlib only, fixtures stub + UDS forwarder + JSON helpers), rustup with repo-local RUSTUP_HOME at .build-artifacts/rustup (targets must be added with that env set, not the default rustup home), cbindgen (self-installed repo-locally by build-herdr-core.sh), cargo-deny + jq (Vendor/herdr/check.sh), system OpenSSH/curl/openssl/unzip, and system Ruby for scripts/*.rb. Postgres is embedded in `coder server` (no brew postgresql, no Docker anywhere); zig is referenced by no script, only by the h16 known-limitation note. cargo-audit and cargo-fuzz are hardening-only and absent on this machine. Existing README's later "Dependencies" section was vendored-library inventory, so it was retitled "Vendored Libraries" to let the new toolchain section own the "Dependencies" heading.
